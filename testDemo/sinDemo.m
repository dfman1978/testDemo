...
-(void)startWave{
    ...
    _shapLayer.borderWidth = 2;
    _shapLayer.borderColor = [UIColor redColor].CGColor;
...
-(void)getLayerPath{
    ...
    CGFloat currentWavePointY = self.frame.size.height;
    CGFloat y = currentWavePointY/2;
    CGFloat waterWaveWidth = self.frame.size.width;
    CGFloat waveCycle = 0.01;
    CGPathMoveToPoint(path, nil, 0, y);
    for (float x = 0.0f; x <=  self.frame.size.width ; x++) {
        // 正弦波浪公式
        
        y -= num * sin(waveCycle*x+10);
        //NSLog(@"y is %f",y);
